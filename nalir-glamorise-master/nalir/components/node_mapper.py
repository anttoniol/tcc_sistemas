...
class NodeMapper:
    ...
    def phrase_process(query, db, config, tokens = ''):        
        if tokens == '':
            tokens = f'{config.zfiles_path}/tokens_adapted.xml'
            tokens = parse(tokens)
    ...
    def tokenizer(query, tokens):
        ...
        while cur_size != len(parse_tree.all_nodes):
            ...
            while i < len(parse_tree.all_nodes):
                ...
                elif is_numeric(cur_node.label) or cur_node.pos == "NUM":
                    cur_node.token_type = 'VT'

                elif cur_node.token_type == 'NA' and cur_node.pos in ['NOUN', 'PROPN'] and cur_node.relationship in ["flat:name", "obj"]: # or cur_node.relationship == "xcomp" or cur_node.pos == 'CD'):
                    cur_node.token_type = 'NTVT'

                elif cur_node.token_type == 'NA' and cur_node.pos.startswith('ADJ'):
                ...
                #elif cur_node.token_type == 'NA' and cur_node.label in ["onde", "aonde"]:
                #    cur_node.token_type = 'SN'
                i+=1
        #print("TERMINOU NODE_MAPPER.TOKENIZER()")
                        ...
                        for k in range(len(phrase_words)):
                            if j != k:
                                if tree.search_node_by_order(index -j + k) is not None:
                                    node_to_delete = tree.search_node_by_order(index-j+k)
    ...
    def delete_useless(query):
        ...
        #    if nlp(word)[0].pos_ == "ADP":
        #        preps.append(word.lower())
        parse_tree =  query.parse_tree
        ...
        while i < len(parse_tree.all_nodes):
            ...
            if parse_tree_node.token_type == 'NA' or parse_tree_node.token_type == 'QT':   
                ...
                if cur_node.label in ["em", "na", "no", "nas", "nos"] and len(cur_node.children):
    ...
    def group_ranking_to_df(text):
        ...
        print("group_ranking_to_df: ", text)
        if text is None:
            return
    ...
    def delete_no_match(query):
        ...
        for i in range(len(parse_tree.all_nodes)):
            ...
            if tree_node.token_type == 'NA':
                cur_node = tree_node
                parse_tree.delete_node(cur_node)
                if cur_node.label in ['em', "no", "na", "nos", "nas"]:
