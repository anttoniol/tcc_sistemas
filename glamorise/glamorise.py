...
class CompoundMerger(Merger):
    def __init__(self, nlp):
        ...
        if compound_pattern_dep and compound_pattern_of:
            self.matcher.add("CompoundMerger", [compound_pattern_dep] + [compound_pattern_of], on_match=None)
        elif compound_pattern_dep:
            self.matcher.add("CompoundMerger", [compound_pattern_dep], on_match=None)
        elif compound_pattern_of:
            self.matcher.add("CompoundMerger", [compound_pattern_of], on_match=None)
        ...
        if config_glamorise.get('units_of_measurement'):
            for unit_of_measurement in config_glamorise['units_of_measurement']:
                ...
                self.matcher.add(
                    ...
                    [units_of_measurement_pattern], on_match=None
                )
...
class Glamorise(metaclass=abc.ABCMeta):
    ...
    def __init__(self, lang="pt_core_news_md", config_glamorise_param="", config_glamorise_interface_param=''):
        ...
        def compound_merger(nlp, name):
            return self.__compound_merger

        def units_of_measurement_merger(nlp, name):
            return self.__units_of_measurement_merger

        Language.factory("compound_merger", func=compound_merger)
        Language.factory("units_of_measurement_merger", func=units_of_measurement_merger)

        self.__nlp.add_pipe("units_of_measurement_merger", last=True)  # Add component to the pipeline
        self.__nlp.add_pipe("compound_merger", last=True)  # Add component to the pipeline

        self.__matcher = Matcher(self.__nlp.vocab, validate=True)
    ...
    def initialize_and_reset_attr(self):
        ...
        self._timer_nlidb_execution_first_and_second_attempt = Timer(
            name="timer_nlidb_execution_first_and_second_attempt", logger=None)
    ...
    def __build_field(self, span, field, pos='', unit_of_measurement=False, time_scale_group_by_field='',
                      replace_text=''):
        ...
        for key, value in config_glamorise['config_glamorise'].items():
            if isinstance(value['reserved_words'], list):
                for reserved_words in value['reserved_words']:
                    ...
                    if "split_words" in value and value["split_words"] == False:
                        reserved_words_pattern = reserved_words_pattern + [{'LOWER': reserved_words.lower()}]
                    else:
                    ...
                    elif "use_default_pattern" in value and value["use_default_pattern"] == False:
                        final_pattern = reserved_words_pattern
                    elif "morphology" in value and value["morphology"] == "ADJ":
                        final_pattern = config_glamorise['default_pattern'] + reserved_words_pattern
                    else:
                        final_pattern = reserved_words_pattern + config_glamorise['default_pattern']
                    self.__matcher.add(key + ' | ' + reserved_words, [final_pattern], on_match=self.collect_sents)
    ...
    def collect_sents(self, matcher, doc, i, matches):
        ...
        if config_glamorise['config_glamorise'][pattern].get('pre_cut_text') and \
                config_glamorise['config_glamorise'][pattern]['pre_cut_text']:
            ...
            if config_glamorise['config_glamorise'][pattern].get(option['json_var']):
                if isinstance(config_glamorise['config_glamorise'][pattern][option['json_var']], list):
                    label = option['label'] + " " + config_glamorise['config_glamorise'][pattern][option['json_var']][
                        index]
                    getattr(self, "_" + option['json_var']).append(
                        config_glamorise['config_glamorise'][pattern][option['json_var']][index])
                elif isinstance(config_glamorise['config_glamorise'][pattern][option['json_var']], str):
                    label = option['label'] + " " + config_glamorise['config_glamorise'][pattern][option['json_var']]
                    getattr(self, "_" + option['json_var']).append(
                        config_glamorise['config_glamorise'][pattern][option['json_var']])
        ...
        if config_glamorise['config_glamorise'][pattern].get('pre_aggregation_functions'):
            ...
            if config_glamorise['config_glamorise'][pattern].get('use_replace_text_as_group_by') and \
                    config_glamorise['config_glamorise'][pattern]['use_replace_text_as_group_by']:
                self.__build_field(span, 'pre_subquery_group_by_fields',
                                   time_scale_group_by_field=
                                   config_glamorise['config_glamorise'][pattern]['pre_subquery_replace_text'][
                                       replace_text_token],
        ...
        if config_glamorise['config_glamorise'][pattern].get('remove_external_group_by') and \
                config_glamorise['config_glamorise'][pattern]['remove_external_group_by']:
    ...
    def __prepare_query_to_NLIDB(self):
        ...
        if config_glamorise.get('pre_after_replace_text'):
            self._pre_prepared_query = self.__replace_text(config_glamorise['pre_after_replace_text'],
                                                           self._pre_prepared_query)
    ...
    def __lemmatize_fields(self, fields):
        for i in range(len(fields)):
            ...
            for field_token in splited_field:
                field_token = self.__nlp(field_token)[0].lemma_
                field += field_token + ' '
            field = field.strip()
            fields[i] = field
    ...
    def _pos_adjust_aggregation_functions_and_fields(self, columns):
        ...
        self.__replace_count_by_sum_in_numeric_fields(columns, self._pos_aggregation_functions,
                                                      self._pos_aggregation_fields)
        # include in sub_query count when attribute is text and has an aggregation function of max, min or avg
        self.__add_subquery_if_max_min_in_text_field(columns, self._pos_aggregation_functions,
                                                     self._pos_aggregation_fields)
        # changing from count to sum in numeric fields
        self.__replace_count_by_sum_in_numeric_fields(columns, self._pos_subquery_aggregation_functions,
                                                      self._pos_subquery_aggregation_fields)
    ...
    def __add_subquery_if_max_min_in_text_field(self, columns, aggregation_functions, aggregation_fields):
        for i in range(len(aggregation_functions)):
            ...
            if aggregation_fields[i] in column_dict:
                if column_dict[aggregation_fields[i]] in ['TEXT', 'BLOB'] and aggregation_functions[i] in ['max', 'min',
                                                                                                           'avg']:
    ...
    def __distinct(self, aggregation_function):
        ...
        if config_glamorise.get('count_with_distinct') and config_glamorise[
            'count_with_distinct'] and aggregation_function == 'count':
            return 'DISTINCT '
        else:
            return ''

    def __prepare_aggregate_SQL(self, columns, limit=100):
        # initialize clauses
        if self._pre_aggregation_fields:
            self.__select_clause = 'SELECT '
        # GLAMORISE has nothing to do, not an aggregation query
        else:
            self.__pos_glamorise_sql = 'SELECT * FROM NLIDB_result_set ' + 'LIMIT ' + str(limit)
            return
        ...
        for i in range(len(self._pos_aggregation_functions)):
            ...
            if self._pos_aggregation_fields[i] in self._pos_subquery_aggregation_fields:
                ...
                self.__from_clause = ' FROM (SELECT ' + ', '.join(
                    nested_group_by_field + [self._pos_subquery_aggregation_functions[j] + '(' + \
                                             distinct + self._pos_aggregation_fields[i] + ') as ' + \
                                             self._pos_aggregation_fields[i]]) + ' FROM NLIDB_result_set'
                self.__from_clause += ' GROUP BY ' + ', '.join(
                    nested_group_by_field + self._pre_subquery_group_by_fields) + ') '
        ...
        for i in range(len(self._pre_having_fields)):
            if self._pre_having_fields[i] in self.pos_aggregation_fields:
                ...
                aggregation_function = self._pos_aggregation_functions[
                    self.pos_aggregation_fields.index(self._pre_having_fields[i])]
            else:
            ...
            self.__having_clause += aggregation_function + \
                                    '(' + distinct + self._pre_having_fields[i] + ') ' + self._pre_having_conditions[
                                        i] + ' ' \
    ...
    def print_timers(self, json=False):
        ...
        print("timer_nlidb_execution_first_and_second_attempt: {:.2f} sec".format(
            self._timer_nlidb_execution_first_and_second_attempt.last))
        result += "</br>timer_nlidb_execution_first_and_second_attempt: {:.2f} sec".format(
            self._timer_nlidb_execution_first_and_second_attempt.last)
        print("timer_nlidb_execution_third_attempt: {:.2f} sec".format(self._timer_nlidb_execution_third_attempt.last))
        result += "</br>timer_nlidb_execution_third_attempt: {:.2f} sec".format(
            self._timer_nlidb_execution_third_attempt.last)
