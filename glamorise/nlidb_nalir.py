...
from nalir.config.config_handler import ConfigHandler
from nalir.rdbms.rdbms import RDBMS
...
class NlidbNalir(NlidbBase):
    ...
    def __include_fields(self, additional_fields):
        ...
        self.__sql = result_group_1 + result_group_2 + ', '.join(fields) + '\n'
    ...
    def execute_query(self, nlq, timer_nlidb_execution_first_and_second_attempt, timer_nlidb_execution_third_attempt, timer_nlidb_json_result_set, nlidb_attempt_level, fields):           
        ...
        try:
            ...
            result_set = json.dumps(result_set, cls=DecimalEncoder)
    ...
    def __join_sql(self, new_sql):
        ...
        select_new_fields, select_new_result, sql_new_list = self._get_fields_in_sql(self.__third_attempt_sql,'(SELECT )(DISTINCT )*(.*)$', 0, 3, ',')
        from_new_fields, from_new_result, sql_new_list = self._get_fields_in_sql(self.__third_attempt_sql, '(FROM )(.*)$', 1, 2,',')
        where_new_fields, where_new_result, sql_new_list = self._get_fields_in_sql(self.__third_attempt_sql, '(WHERE )(.*)$', 2, 2,' AND ')

        select_final_fields = list(dict.fromkeys(select_new_sql_fields + select_fields + select_new_fields))
        from_final_fields = list(dict.fromkeys(from_new_sql_fields + from_fields + from_new_fields))
        where_final_fields = list(dict.fromkeys(where_fields + where_new_fields))

        select_str = "SELECT " + ', '.join(select_final_fields)
